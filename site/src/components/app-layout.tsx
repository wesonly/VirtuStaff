...
function NavIcon({ icon }: { icon: string }) {
  const paths: Record<string, string> = {
    ...
    compass: "M12 21a9 9 0 100-18 9 9 0 000 18zm0 0a9 9 0 01-3.44-17.32M12 21a9 9 0 003.44-17.32M12 21V3m0 9a2.25 2.25 0 100-4.5A2.25 2.25 0 0012 12zm0 0a2.25 2.25 0 004.5 0A2.25 2.25 0 0012 12z",
  ...
  return (
    <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d={paths[icon] || paths.dashboard} />
    </svg>
  );
}
...
export function AppLayout() {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-gray-200 bg-white transition-transform duration-300 dark:border-gray-800 dark:bg-gray-900 lg:static lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        {/* Logo */}
        <div className="flex h-16 items-center gap-3 border-b border-gray-200 px-6 dark:border-gray-800">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500 to-violet-500 text-sm font-bold text-white">V</div>
          <span className="text-lg font-bold tracking-tight text-gray-900 dark:text-white">VirtuStaff</span>
        </div>
        ...
        <nav className="flex-1 space-y-1 p-4">
          {navItems.map((item) => (
            ...
            >
              ...
              {item.soon && <span className="ml-auto rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-medium text-gray-500 dark:bg-gray-800 dark:text-gray-400">Soon</span>}
              {item.badge && <span className="ml-auto flex h-2 w-2 rounded-full bg-indigo-500" title="New" />}
            </Link>
          ))}
        </nav>
      ...
      <div className="flex flex-1 flex-col">
        ...
        <header className="flex h-16 items-center justify-between border-b border-gray-200 bg-white px-4 dark:border-gray-800 dark:bg-gray-900 lg:px-6">
          <button onClick={() => setSidebarOpen(true)} className="rounded-lg p-2 text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-gray-200 lg:hidden">
          ...
          <div className="hidden items-center gap-2 lg:flex">
            <span className="text-sm font-medium text-gray-900 dark:text-white">Acme Corp</span>
